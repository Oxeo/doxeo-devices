...
void setup() {
  ...
  Mirf.setRADDR((byte *) DOXEO_ADDR_DOORMAT);
  ...
  // init key interrupt
  attachInterrupt(digitalPinToInterrupt(SWITCH), switchInterruptHandle, CHANGE);
...
void loop() {
  ...
  if (checkNewMsg && Mirf.dataReady()) {
    ...
    // handle message
    if (receptorName != DOXEO_ADDR_DOORMAT) {
...
void sendMessage(String device, String msg) {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    if (device == "") {
      ...
      success = sendNrf(String(DOXEO_ADDR_DOORMAT) + '-' + device + ';' + String(DOXEO_ADDR_MOTHER) + ';' + msg);
    }
